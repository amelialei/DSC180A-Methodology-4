...
The most interesting topic covered in my domain this quarter was the idea of quantifying misinformation through interpretive predictive modeling. 
...
A potential investigation I would like to pursue for my Quarter 2 Project is to extend my current predictive model pipeline into a hybrid
agentic framework that incorporates generative ai, retrieval, and collaboration between different agents. My main goal for this 
...
One potential change I'd like to make to my approach is to scrape more data and aggregate articles from multiple sources. Right now, this project 
...
more time to expand on the data itself as high quality data will also directly improve model accuracy. By doing this, the system would evolve from 
...
There are several techniques I would be interested in exploring in my project. The first one is RAG so that the model can focus on retrieving relevant 
...
the project where the LLM can use tools to call the predictive models when doing its analysis. Lastly, I would also be interested in exploring vector 
databases like ChromaDB as it's something I've never used before. I think this would be really helpful in allowing my model to retain any 
